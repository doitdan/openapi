...
Only the success response is documented by default, so a consumer cannot tell a documented failure from an undocumented one. The codes each operation can actually produce are derived from its shape and added where they are missing.

| Code | Added when |
|---|---|
| 400 | the operation takes a body or parameters |
| 401, 403 | the operation requires a credential |
| 404 | the operation takes a path variable, or is not a `POST` |
| 409 | the operation writes — a conflict needs something to conflict with |
| 500 | always |
...
openapi:
  error-responses:
    include: [400, 401, 403, 404, 409, 500]
    descriptions:
